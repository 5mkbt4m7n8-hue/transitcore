...
// Dedicated ESP32 client for TransitCore LED feed v1.
// This sketch does not replace or modify the Gråkallbanen v0.40.x firmware.
...
void ensureWifi() {
  ...
  Serial.println("Wi-Fi frakoblet. Starter nytt forsøk.");
...
void fetchFrame() {
  ...
  for (int retry = 0; retry <= HTTP_MAX_RETRIES; retry++) {
    if (fetchFrameAttempt(retry)) return;
...
void enforceTtl() {
  ...
  clearHardware();
  Serial.println("LED-frame utløpt. Tavlen er slukket.");
}
...
void setup() {
  ...
  Serial.println("TransitCore Board Client v0.1.0 starter.");
  Serial.printf(
    "Board %s | %u LED-er | hardware %s\n",
    EXPECTED_BOARD_PROFILE,
    LED_COUNT,
    LED_HARDWARE_ENABLED ? "PÅ" : "AV"
  );
}

...
// TransitCore Universal Board Client v1.0.8
// One stable ESP32 engine; board_config.h selects the physical board.
// v1.0.8 reads the versioned signal policy supplied by the Worker.
...
const uint16_t DEFAULT_APPROACH_PULSE_MS = 1800;
...
uint16_t activeApproachPulseMs = DEFAULT_APPROACH_PULSE_MS;
...
uint8_t approachingPulse() {
  const unsigned long period = max((uint16_t)500, activeApproachPulseMs);
...
bool parseAndValidateFrame(
  ...
  approachPulseMs = DEFAULT_APPROACH_PULSE_MS;
  if (!signalPolicy.isNull()) {
    ...
    if (pulseMs < 500 || pulseMs > 10000) {
      error = "ugyldig approachPulseMs";
      return false;
    }
...
bool fetchFrameAttempt(int retryIndex) {
  ...
  uint16_t approachPulseMs = DEFAULT_APPROACH_PULSE_MS;
...
void setup() {
  ...
  Serial.println("TransitCore Universal Board Client v1.0.8 starter.");
  Serial.printf(

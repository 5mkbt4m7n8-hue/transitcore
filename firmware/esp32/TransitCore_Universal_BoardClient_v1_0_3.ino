...
// TransitCore Universal Board Client v1.0.3
// One stable ESP32 engine; board_config.h selects the physical board.
// v1.0.3 adds local health reports; frame and LED behavior are unchanged.
...
void startWifiAttempt(bool resetRadio) {
  if (resetRadio) {
    Serial.println("Nullstiller Wi-Fi-radio fÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¸r nytt forsÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¸k.");
  ...
  Serial.printf(
    "Wi-Fi-forsÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¸k %u startet | neste tidligst om %lu s\n",
    wifiAttemptCount,
...
void ensureWifi() {
  ...
  if (now - wifiDisconnectedAtMs >= WIFI_DEVICE_RESTART_MS) {
    Serial.println("Wi-Fi har vÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¦rt borte i 10 minutter. Starter ESP pÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¥ nytt.");
    Serial.flush();
    delay(100);
    ESP.restart();
  }
...
bool parseAndValidateFrame(
  ...
  if (ageSeconds > (int64_t)ttlSeconds) {
    error = "frame er utlÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¸pt ved mottak: " + String((long)ageSeconds) + " s";
    return false;
  }
  ...
  for (JsonObject item : leds) {
    ...
    if (
      ...
      error = "ugyldig LED-oppfÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¸ring";
      return false;
    }
...
void reportHealth() {
  ...
  Serial.printf(
    ...
    "feed OK/feil %lu/%lu | frame %s | alder %lu s | heap %u | min %u\n",
    now / 1000UL,
    ...
    !hasValidFrame ? "INGEN" : (ttlExpired ? "UTLÃƒËœPT" : "OK"),
    frameAgeSeconds,
    freeHeap,
    minimumFreeHeap
  );
...
void enforceTtl() {
  ...
  clearHardware();
  Serial.println("LED-frame utlÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¸pt. Tavlen er slukket.");
}
...
void setup() {
  ...
  Serial.println("TransitCore Universal Board Client v1.0.3 starter.");
  Serial.printf(
    "Board %s | %u LED-er | hardware %s\n",
    EXPECTED_BOARD_PROFILE,
    LED_COUNT,
    LED_HARDWARE_ENABLED ? "PÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦" : "AV"
  );
}

...
// TransitCore Universal Board Client v1.0.4
// One stable ESP32 engine; board_config.h selects the physical board.
// v1.0.4 adds authenticated remote health reports; LED behavior is unchanged.
...
void sendHealthStatus(unsigned long now, uint32_t freeHeap) {
  if (WiFi.status() != WL_CONNECTED || strlen(TRANSITCORE_STATUS_TOKEN) == 0) return;

  DynamicJsonDocument document(768);
  document["schemaVersion"] = 1;
  document["boardProfile"] = EXPECTED_BOARD_PROFILE;
  document["firmware"] = "1.0.4";
...
void setup() {
  ...
  Serial.println("TransitCore Universal Board Client v1.0.4 starter.");
  Serial.printf(

...
#error "TransitCore 1.2.12 requires Arduino ESP32 core 3.3.11 or newer for OTA trial boot"
#endif
enum LedState : uint8_t;
struct LedPixel;

// TransitCore Universal Board Client v1.2.12
...
const char* TRANSITCORE_FIRMWARE_VERSION = "1.2.12";
...
void printFrameSimulation(uint32_t sequence, uint16_t activeCount) {
  ...
  for (uint16_t id = 0; id < LED_COUNT; id++) {
    ...
    const String direction = candidateDirectionLabels[id].length() > 0
      ...
      : "retning ukjent";
    diagnosticSerial.printf(
      "  LED %03u | %-11s | %-28s | RGB %u,%u,%u | brightness %u\n",
      id,
...
bool parseAndValidateFrame(
  ...
  for (uint16_t id = 0; id < LED_COUNT; id++) candidateDirectionLabels[id] = "";
...
void setup() {
  ...
  diagnosticSerial.printf("TransitCore Universal Board Client v1.2.12 starter | kompatibel stabil ESP32-S3 LED-driver | hardwareprofil låser LED-datapin | vedvarende feilkø %u/10 | Wi-Fi-reset: hold BOOT i 5 sekunder.\n", deviceErrorQueueCount);
  diagnosticSerial.printf(

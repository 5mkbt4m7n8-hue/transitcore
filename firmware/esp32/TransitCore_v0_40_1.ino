...
// USER CONFIG
// -----------------------------------------------------------------------------
// RGB hardware is opt-in. When false, the NeoPixel object is never begun and
// no signal is sent to RGB_DATA_PIN. The existing 16 red GPIO LEDs are unchanged.
const bool RGB_HARDWARE_ENABLED = false;
const bool RGB_BENCH_TEST_ENABLED = false;
const uint8_t RGB_DATA_PIN = 2;
const neoPixelType RGB_PIXEL_TYPE = NEO_GRB + NEO_KHZ800;

// Overall output cap for a safe first bench test (0..255). Each VLED's existing
// logical brightness and animation intensity are applied inside this limit.
const uint8_t RGB_HARDWARE_BRIGHTNESS_LIMIT = 64;
const uint8_t RGB_BENCH_BRIGHTNESS = 20;

const unsigned long RGB_BLINK_INTERVAL_MS = 500;
const unsigned long RGB_PULSE_PERIOD_MS = 2000;
const unsigned long RGB_BENCH_STEP_INTERVAL_MS = 600;

// Physical installation note: use a separate correctly sized 5 V supply and
// common ground. Add a supply capacitor, a series data resistor, and consider a
// 3.3 V -> 5 V level shifter. Never power the full chain from the ESP32 regulator.
...
StationLed stations[] = {
  ...
  {"Herlofsonløypa", 63.405185, 10.320049, 17},
  ...
  {"Søndre Hoem",    63.403350, 10.381672, 26},
...
// -----------------------------------------------------------------------------
void connectWifi() {
  Serial.println("Kobler til konfigurert Wi-Fi...");
  ...
  while (WiFi.status() != WL_CONNECTED) {
    ...
    if (millis() - startedAt > 20000) {
      Serial.println();
      Serial.println("Wi-Fi timeout. Starter nytt forsøk.");
...
// The optional hardware driver consumes this frame without changing its rules.
...
// -----------------------------------------------------------------------------
// ADDRESSABLE RGB HARDWARE DRIVER v1
// -----------------------------------------------------------------------------
// Hardware output is a final, one-way consumer of the existing RGB Render
// Frame. It contains no position, direction, conflict or vehicle-state logic.

Adafruit_NeoPixel rgbStrip;
bool rgbHardwareReady = false;

enum RgbBenchPhaseCode {
  RGB_BENCH_CHANNEL_RED = 0,
  RGB_BENCH_CHANNEL_GREEN = 1,
  RGB_BENCH_CHANNEL_BLUE = 2,
  RGB_BENCH_LED_ORDER = 3,
  RGB_BENCH_FINISH = 4
};

bool rgbBenchRunning = false;
bool rgbBenchFinished = false;
int rgbBenchPhase = RGB_BENCH_CHANNEL_RED;
int rgbBenchLedIndex = 0;
unsigned long rgbBenchNextStepAt = 0;

uint8_t rgbAnimationIntensity(
  int renderMode,
  unsigned long now
) {
  if (renderMode == RGB_RENDER_BLINK) {
    return ((now / RGB_BLINK_INTERVAL_MS) % 2UL) == 0UL
      ? 255
      : 0;
  }

  if (renderMode == RGB_RENDER_PULSE) {
    const unsigned long halfPeriod = RGB_PULSE_PERIOD_MS / 2UL;
    const unsigned long phase = now % RGB_PULSE_PERIOD_MS;
    const unsigned long ramp = phase < halfPeriod
      ? phase
      : RGB_PULSE_PERIOD_MS - phase;

    // Gentle pulse from 32/255 to full animation intensity.
    return (uint8_t)(32UL + (ramp * 223UL) / halfPeriod);
  }

  return 255;
}

uint8_t scaleRgbChannel(
  uint8_t channel,
  uint8_t intensity
) {
  return (uint8_t)(
    ((uint16_t)channel * (uint16_t)intensity + 127U) / 255U
  );
}

void startRgbBenchTest() {
  if (
    !RGB_HARDWARE_ENABLED ||
    !RGB_BENCH_TEST_ENABLED ||
    !rgbHardwareReady
  ) {
    return;
  }

  rgbStrip.setBrightness(RGB_BENCH_BRIGHTNESS);
  rgbStrip.clear();
  rgbStrip.show();

  rgbBenchPhase = RGB_BENCH_CHANNEL_RED;
  rgbBenchLedIndex = 0;
  rgbBenchNextStepAt = millis();
  rgbBenchRunning = true;
  rgbBenchFinished = false;

  Serial.println();
  Serial.println("RGB Bench Test starter");
  Serial.printf("LED count: %d\n", totalVirtualLedCount());
  Serial.printf("Data pin: %u\n", RGB_DATA_PIN);
  Serial.printf("Brightness: %u\n", RGB_BENCH_BRIGHTNESS);
}

void updateRgbBenchTest() {
  if (!rgbBenchRunning || rgbBenchFinished) {
    return;
  }

  const unsigned long now = millis();
  if ((long)(now - rgbBenchNextStepAt) < 0) {
    return;
  }

  rgbBenchNextStepAt = now + RGB_BENCH_STEP_INTERVAL_MS;
  rgbStrip.clear();

  if (rgbBenchPhase == RGB_BENCH_CHANNEL_RED) {
    rgbStrip.setPixelColor(0, 255, 0, 0);
    rgbStrip.show();
    Serial.println("RGB channel test | LED 0 | RED");
    rgbBenchPhase = RGB_BENCH_CHANNEL_GREEN;
    return;
  }

  if (rgbBenchPhase == RGB_BENCH_CHANNEL_GREEN) {
    rgbStrip.setPixelColor(0, 0, 255, 0);
    rgbStrip.show();
    Serial.println("RGB channel test | LED 0 | GREEN");
    rgbBenchPhase = RGB_BENCH_CHANNEL_BLUE;
    return;
  }

  if (rgbBenchPhase == RGB_BENCH_CHANNEL_BLUE) {
    rgbStrip.setPixelColor(0, 0, 0, 255);
    rgbStrip.show();
    Serial.println("RGB channel test | LED 0 | BLUE");
    rgbBenchPhase = RGB_BENCH_LED_ORDER;
    return;
  }

  if (rgbBenchPhase == RGB_BENCH_LED_ORDER) {
    const int ledCount = totalVirtualLedCount();

    if (rgbBenchLedIndex < ledCount) {
      // Only one LED is active. Global bench brightness remains capped at 20.
      rgbStrip.setPixelColor(
        (uint16_t)rgbBenchLedIndex,
        255,
        120,
        0
      );
      ...
      if (rgbBenchLedIndex >= ledCount) {
        rgbBenchPhase = RGB_BENCH_FINISH;
      }
      return;
    }
  ...
  // Safest post-test behavior: keep the chain off until the next restart.
  rgbStrip.show();
  rgbBenchRunning = false;
  rgbBenchFinished = true;
  Serial.println("RGB Bench Test ferdig");
  Serial.println("Alle RGB-LED-er slukket");
}

void setupRgbHardware() {
  const int ledCount = totalVirtualLedCount();

  Serial.println("RGB Hardware Driver v1");
  Serial.printf(
    "enabled: %s\n",
    RGB_HARDWARE_ENABLED ? "YES" : "NO"
  );
  Serial.printf("LED count: %d\n", ledCount);
  Serial.printf("data pin: %u\n", RGB_DATA_PIN);

  if (!RGB_HARDWARE_ENABLED) {
    // Deliberately do not call setPin(), begin() or show(). This guarantees
    // that the disabled driver sends no signal to RGB_DATA_PIN.
    return;
  }

  Serial.printf(
    "brightness limit: %u\n",
    RGB_HARDWARE_BRIGHTNESS_LIMIT
  );

  rgbStrip.updateType(RGB_PIXEL_TYPE);
  rgbStrip.updateLength((uint16_t)ledCount);
  rgbStrip.setPin((int16_t)RGB_DATA_PIN);
  rgbStrip.begin();
  rgbStrip.setBrightness(RGB_HARDWARE_BRIGHTNESS_LIMIT);

  // Required startup state: all addressable LEDs off before the first valid
  // Entur/VLED frame. This is the only startup transmission.
  rgbStrip.clear();
  rgbStrip.show();
  rgbHardwareReady = true;
  startRgbBenchTest();
}

void updateRgbHardware() {
  if (!RGB_HARDWARE_ENABLED || !rgbHardwareReady) {
    return;
  }

  if (RGB_BENCH_TEST_ENABLED) {
    updateRgbBenchTest();
    return;
  }
  ...
  for (int i = 0; i < ledCount; i++) {
    if (!virtualLedActive[i]) {
      rgbStrip.setPixelColor((uint16_t)i, 0);
      continue;
    }

    const uint8_t animationIntensity = rgbAnimationIntensity(
      virtualLedRenderMode[i],
      now
    );
    const uint8_t combinedIntensity = (uint8_t)(
      ((uint16_t)virtualLedBrightness[i] *
       (uint16_t)animationIntensity + 127U) / 255U
    );

    rgbStrip.setPixelColor(
      (uint16_t)i,
      scaleRgbChannel(virtualLedRed[i], combinedIntensity),
      scaleRgbChannel(virtualLedGreen[i], combinedIntensity),
      scaleRgbChannel(virtualLedBlue[i], combinedIntensity)
    );
  }
...
bool fetchAndRenderEntur() {
  ...
  for (
    ...
    Serial.println();
    Serial.printf(
      "Henter Entur | retry %d/%d | free heap før HTTP: %u bytes\n",
      retryNumber,
    ...
    Serial.printf(
      "Entur body mottatt | %u bytes | ferdig: %s | receive result: %d | free heap før/etter: %u/%u bytes\n",
    ...
    Serial.printf(
      "Entur body komplett | %u bytes | free heap før parse: %u bytes\n",
      (unsigned)receivedBodyBytes,
      ESP.getFreeHeap()
    );
  ...
  for (JsonObject vehicle : vehicles) {
    ...
    if (existingIndex < 0) {
      if (uniqueCount >= MAX_ACTIVE_VEHICLES) {
        Serial.println(
          "ADVARSEL: MAX_ACTIVE_VEHICLES nådd."
        );
        continue;
      }
    ...
    if (freshness > 0) {
      ...
      if (conflict) {
        Serial.println(
          "  -> DATAKONFLIKT: samme lastUpdated, ulike data. Beholder første post."
        );
      } else {
        Serial.println(
  ...
  Serial.printf(
    "Rå linje-9 poster: %d | duplikater: %d | unike vogner: %u\n",
    rawLine9Count,
    duplicateCount,
  ...
  Serial.printf(
    "Aktive Gråkallbane-vogner etter dedup/freshness: %d | stale ignorert: %d\n",
    tramCount,
    staleCount
  );
...
void setup() {
  ...
  Serial.println();
  Serial.println("TransitCore ESP32 v0.40.0");

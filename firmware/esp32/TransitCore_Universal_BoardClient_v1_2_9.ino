...
// TransitCore Universal Board Client v1.2.9
...
const char* TRANSITCORE_FIRMWARE_VERSION = "1.2.9";
...
const unsigned long LED_BUS_RECOVERY_INTERVAL_MS = 30UL * 1000UL;
...
void recoverLedBus(const char* reason) {
  if (!LED_HARDWARE_ENABLED) return;
  if (ledHardwareMutex != nullptr) xSemaphoreTake(ledHardwareMutex, portMAX_DELAY);
  // If the strip loses 5 V while the ESP remains powered, restart the data
  // transport with a defined low/reset period before retransmitting the frame.
  ...
  ledBusRecoveryCount++;
  Serial.printf("LED-BUSS GJENOPPRETTET | grunn %s | antall %lu\n", reason, (unsigned long)ledBusRecoveryCount);
}
...
void handleSerialCommand(String command) {
  ...
  if (command == "AMBIENT ON") setAmbientLighting(true);
    ...
    Serial.println("KOMMANDOER | AMBIENT ON/OFF/TOGGLE | TRAINS WARM/LINE | MODE NEXT/STATUS | BRIGHTNESS NEXT/100/50/10 | LED RECOVER");
  } else {
...
void ledRenderTask(void* parameter) {
  while (true) {
    ...
    if (hasValidFrame && !ttlExpired && now - lastValidFrameAtMs > frameTtlMs) {
      ...
      portEXIT_CRITICAL(&frameMutex);
      clearHardware();
      Serial.println("LED-frame utløpt i LED-oppgaven. Tavlen er slukket uten å vente på nettverket.");
    }
    if (
      !provisioningActive && !ledTestActive && !startupWaveActive &&
      now - lastLedBusRecoveryAtMs >= LED_BUS_RECOVERY_INTERVAL_MS
    ) {
      recoverLedBus("PERIODISK VAKTHUND");
    }
    if (
      !provisioningActive &&
      !ledTestActive &&
...
void setup() {
  ...
  Serial.printf("TransitCore Universal Board Client v1.2.9 starter | hardwareprofil låser LED-datapin | LED-vakthund og strømretur | vedvarende feilkø %u/10 | Wi-Fi-reset: hold BOOT i 5 sekunder.\n", deviceErrorQueueCount);
  Serial.printf(

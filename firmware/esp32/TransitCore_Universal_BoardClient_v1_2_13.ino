...
#error "TransitCore 1.2.13 requires Arduino ESP32 core 3.3.11 or newer for OTA trial boot"
#endif
enum LedState : uint8_t;
struct LedPixel;

// TransitCore Universal Board Client v1.2.13
...
// v1.2.5 keeps a persistent ten-event error queue and delivers it after reconnect.
// v1.2.6 includes vehicle direction/destination in the Serial LED status log.
// v1.2.7 self-heals after strip power/data loss and enforces TTL off-loop.
// v1.2.8 makes the configured LED data GPIO visible at startup.
// v1.2.9 locks generated packages to the current hardware-profile GPIO.
// v1.2.10 removes destructive periodic NeoPixel reinitialization on ESP32-S3.
// v1.2.11 keeps manual recovery compatible with all Adafruit NeoPixel releases.
// v1.2.12 adds bounded wireless diagnostics and device-targeted OTA.
// v1.2.13 adds vehicle IDs to the Serial LED status log.
...
const char* OTA_CHIP = "esp32s3";
#elif CONFIG_IDF_TARGET_ESP32
const char* OTA_CHIP = "esp32";
#else
const char* OTA_CHIP = "unsupported";
#endif

// Bounded RAM-only diagnostics. Never capture credentials or provisioning input.
// The LED task never waits for the log mutex; contention drops diagnostics only.
class DiagnosticSerial : public Print {
  static const size_t COUNT = 48, WIDTH = 192;
  char lines[COUNT][WIDTH] = {};
  uint32_t ids[COUNT] = {};
  uint32_t times[COUNT] = {};
  char partial[WIDTH] = {};
...
public:
  ...
  size_t write(const uint8_t* data, size_t size) override {
    ...
    for (size_t i=0; i<size; ++i) {
      ...
      if (c == '\n') {
        partial[used] = 0;
        const char* allowed[] = {"LED-", "  LED ", "  Ingen", "TAVLESIMULATOR",
          "HELSE", "STATUS", "OTA", "FEIL |", "Henter LED-feed", "HTTP ", "Feed-", "Wi-Fi frakoblet"};
        bool keep = false;
        for (const char* prefix : allowed)
          if (strncmp(partial, prefix, strlen(prefix)) == 0) keep = true;
        if (keep) {
          snprintf(lines[head], WIDTH, "%s", partial);
          ids[head] = ++sequence;
          times[head] = millis()/1000UL;
          head = (head + 1) % COUNT;
        }
        used = 0;
      } else if (c != '\r' && used < WIDTH-1) partial[used++] = c;
    }
    xSemaphoreGive(mutex);
    return size;
  }
  uint32_t snapshot(JsonArray out, uint32_t after) {
    if (!mutex || xSemaphoreTake(mutex, 0) != pdTRUE) return after;
    uint32_t last = after;
    for (size_t i=0; i<COUNT && out.size()<24; ++i) {
      const size_t at = (head+i)%COUNT;
      if (ids[at] > after) {
        JsonObject row = out.createNestedObject();
        row["seq"] = ids[at]; row["text"] = lines[at]; row["uptimeSeconds"] = times[at]; last = ids[at];
      }
    }
    xSemaphoreGive(mutex);
    return last;
...
// Arduino ESP32 3.3.11 supports deferred confirmation of an OTA trial boot.
#if CONFIG_APP_ROLLBACK_ENABLE
extern "C" bool verifyRollbackLater() { return true; }
#endif
bool otaTrialBoot = false;
volatile unsigned long lastLedTaskAliveMs = 0;
uint32_t remoteLogUntilMs = 0, remoteLogLastAttemptMs = 0, remoteLogSent = 0;
String diagnosticBootId;
void acceptLogLease(const String& body) {
  DynamicJsonDocument response(512);
  if (!deserializeJson(response, body)) {
    const uint32_t seconds = min(uint32_t(response["logSeconds"] | 0U), uint32_t(900));
    remoteLogUntilMs = seconds ? millis() + seconds*1000UL : 0;
  }
}
...
const unsigned long HTTP_CONNECT_TIMEOUT_MS = 7000;
const unsigned long HTTP_RESPONSE_TIMEOUT_MS = 7000;
...
const char* TRANSITCORE_FIRMWARE_VERSION = "1.2.13";
...
String candidateDirectionLabels[LED_COUNT];
String candidateVehicleIds[LED_COUNT];
...
volatile bool ttlExpiryReportPending = false;
uint32_t lastSequence = 0;
volatile uint32_t frameTtlMs = 30000;
volatile unsigned long lastValidFrameAtMs = 0;
unsigned long lastLedBusRecoveryAtMs = 0;
uint32_t ledBusRecoveryCount = 0;
...
const uint8_t DEVICE_ERROR_QUEUE_CAPACITY = 10;
struct DeviceErrorEvent {
  uint32_t id;
  String code;
  String detail;
  uint32_t occurredAtUptimeSeconds;
  uint32_t occurrences;
};
DeviceErrorEvent deviceErrorQueue[DEVICE_ERROR_QUEUE_CAPACITY];
uint8_t deviceErrorQueueCount = 0;
uint32_t nextDeviceErrorId = 1;

// -----------------------------------------------------------------------------
// HELPERS
// -----------------------------------------------------------------------------
void persistDeviceErrorQueue() {
  DynamicJsonDocument document(3072);
  JsonArray events = document.to<JsonArray>();
  for (uint8_t index = 0; index < deviceErrorQueueCount; index++) {
    JsonObject event = events.createNestedObject();
    event["id"] = deviceErrorQueue[index].id;
    event["code"] = deviceErrorQueue[index].code;
    event["detail"] = deviceErrorQueue[index].detail;
    event["occurredAtUptimeSeconds"] = deviceErrorQueue[index].occurredAtUptimeSeconds;
    event["occurrences"] = deviceErrorQueue[index].occurrences;
  }
  String encoded;
  serializeJson(document, encoded);
  errorPreferences.begin("tc-errors", false);
  errorPreferences.putUInt("nextId", nextDeviceErrorId);
  if (deviceErrorQueueCount == 0) errorPreferences.remove("queue");
  else errorPreferences.putString("queue", encoded);
  errorPreferences.end();
}

void loadDeviceErrorQueue() {
  errorPreferences.begin("tc-errors", true);
  nextDeviceErrorId = errorPreferences.getUInt("nextId", 1);
  const String encoded = errorPreferences.getString("queue", "");
  errorPreferences.end();
  if (nextDeviceErrorId == 0) nextDeviceErrorId = 1;
  if (encoded.length() == 0) return;
  DynamicJsonDocument document(3072);
  if (deserializeJson(document, encoded)) return;
  for (JsonObject event : document.as<JsonArray>()) {
    if (deviceErrorQueueCount >= DEVICE_ERROR_QUEUE_CAPACITY) break;
    const char* code = event["code"] | "";
    const char* detail = event["detail"] | "";
    if (code[0] == '\0') continue;
    DeviceErrorEvent& stored = deviceErrorQueue[deviceErrorQueueCount++];
    stored.id = event["id"] | 0;
    stored.code = code;
    stored.detail = detail;
    stored.occurredAtUptimeSeconds = event["occurredAtUptimeSeconds"] | 0;
    stored.occurrences = event["occurrences"] | 1;
  }
  if (deviceErrorQueueCount > 0) {
    const DeviceErrorEvent& latest = deviceErrorQueue[deviceErrorQueueCount - 1];
    lastDeviceErrorCode = latest.code;
    lastDeviceErrorDetail = latest.detail;
    lastDeviceErrorAtUptimeSeconds = latest.occurredAtUptimeSeconds;
    lastDeviceErrorOccurrences = latest.occurrences;
    deviceErrorReportPending = true;
  }
}

void clearDeliveredDeviceErrors() {
  deviceErrorQueueCount = 0;
  lastDeviceErrorCode = "";
  lastDeviceErrorDetail = "";
  lastDeviceErrorAtUptimeSeconds = 0;
  lastDeviceErrorOccurrences = 0;
  persistDeviceErrorQueue();
}
...
void recordDeviceError(const char* code, const String& detail) {
  ...
  safeDetail.replace("\n", " ");
  diagnosticSerial.printf("FEIL | %s | %s\n", code, safeDetail.c_str());
  if (lastDeviceErrorCode == code && lastDeviceErrorDetail == safeDetail) {
    ...
    if (deviceErrorQueueCount > 0) {
      deviceErrorQueue[deviceErrorQueueCount - 1].occurrences = lastDeviceErrorOccurrences;
      deviceErrorQueue[deviceErrorQueueCount - 1].occurredAtUptimeSeconds = millis() / 1000UL;
      // Persist at powers of two to retain useful counts without wearing flash
      // during a rapidly repeating network failure.
      if ((lastDeviceErrorOccurrences & (lastDeviceErrorOccurrences - 1)) == 0) persistDeviceErrorQueue();
    }
  } else {
    lastDeviceErrorCode = code;
    lastDeviceErrorDetail = safeDetail;
    lastDeviceErrorOccurrences = 1;
    if (deviceErrorQueueCount == DEVICE_ERROR_QUEUE_CAPACITY) {
      for (uint8_t index = 1; index < DEVICE_ERROR_QUEUE_CAPACITY; index++) deviceErrorQueue[index - 1] = deviceErrorQueue[index];
      deviceErrorQueueCount--;
    }
    DeviceErrorEvent& event = deviceErrorQueue[deviceErrorQueueCount++];
    event.id = nextDeviceErrorId++;
    if (nextDeviceErrorId == 0) nextDeviceErrorId = 1;
    event.code = lastDeviceErrorCode;
    event.detail = lastDeviceErrorDetail;
    event.occurredAtUptimeSeconds = millis() / 1000UL;
    event.occurrences = 1;
    persistDeviceErrorQueue();
  }
...
void recoverLedBus(const char* reason) {
  if (!LED_HARDWARE_ENABLED) return;
  if (ledHardwareMutex != nullptr) xSemaphoreTake(ledHardwareMutex, portMAX_DELAY);
  // Send a defined reset frame without rebuilding the ESP32 RMT transport.
  // The render task retransmits the current complete frame on its next pass.
  strip.clear();
  strip.show();
  delayMicroseconds(350);
  if (ledHardwareMutex != nullptr) xSemaphoreGive(ledHardwareMutex);
  lastLedBusRecoveryAtMs = millis();
  ledBusRecoveryCount++;
  diagnosticSerial.printf("LED-BUSS GJENOPPRETTET | grunn %s | antall %lu\n", reason, (unsigned long)ledBusRecoveryCount);
}

void printVisualMode() {
  diagnosticSerial.printf(
...
void handleSerialCommand(String command) {
  ...
  else if (command == "LED RECOVER") recoverLedBus("SERIAL");
  else if (command == "OTA CHECK") { otaCheckAttempted = false; diagnosticSerial.println("OTA | kontroll bestilt."); }
  else if (command == "HELP") {
    diagnosticSerial.println("KOMMANDOER | AMBIENT ON/OFF/TOGGLE | TRAINS WARM/LINE | MODE NEXT/STATUS | BRIGHTNESS NEXT/100/50/10 | LED RECOVER | OTA CHECK");
  } else {
    diagnosticSerial.println("Ukjent kommando. Skriv HELP.");
  }
}
...
void showFrozenIsolationFrame() {
  ...
  if (!isolationFrameCaptured) {
    ...
    diagnosticSerial.println("ISOLASJONSTEST | første Worker-frame er fryst | senere frames vises ikke");
  }
  if (ledHardwareMutex != nullptr) xSemaphoreTake(ledHardwareMutex, portMAX_DELAY);
  for (uint16_t i = 0; i < LED_COUNT; i++) {
...
void ledRenderTask(void* parameter) {
  while (true) {
    const unsigned long now = millis();
    if (hasValidFrame && !ttlExpired && now - lastValidFrameAtMs > frameTtlMs) {
      ttlExpired = true;
      ttlExpiryReportPending = true;
      portENTER_CRITICAL(&frameMutex);
      clearFrame(activeFrame);
      portEXIT_CRITICAL(&frameMutex);
      clearHardware();
      diagnosticSerial.println("LED-frame utløpt i LED-oppgaven. Tavlen er slukket uten å vente på nettverket.");
    }
    if (
      !provisioningActive &&
      !ledTestActive &&
...
bool updateLedTest() {
  ...
  if (ledTestStep < 3) {
    ...
    strip.show();
    if (ledHardwareMutex != nullptr) xSemaphoreGive(ledHardwareMutex);
    diagnosticSerial.printf("LED-TEST kanal | LED 0 | %s\n", ledTestStep == 0 ? "RED" : ledTestStep == 1 ? "GREEN" : "BLUE");
    ledTestStep++;
    return true;
  }
  ...
  if (physicalLed < TRANSITCORE_PHYSICAL_LED_COUNT) {
    ...
    strip.show();
    if (ledHardwareMutex != nullptr) xSemaphoreGive(ledHardwareMutex);
    diagnosticSerial.printf("LED-TEST fysisk | LED %u/%u\n", physicalLed, TRANSITCORE_PHYSICAL_LED_COUNT - 1);
    ledTestStep++;
    return true;
  }
  ...
  if (ledHardwareMutex != nullptr) xSemaphoreGive(ledHardwareMutex);
  ledTestActive = false;
  diagnosticSerial.println("LED-TEST ferdig. Alle LED-er er slukket. Normal drift starter.");
  return false;
}
...
void startProvisioning() {
  ...
  if (!WiFi.softAP(provisioningApName.c_str())) {
    diagnosticSerial.println("OPPSETT | Klarte ikke starte Wi-Fi-nettet");
    return;
  }
  ...
  provisioningServer.on("/save", HTTP_POST, []() {
    ...
    diagnosticSerial.printf("OPPSETT | Nettverk lagret | SSID %s | starter på nytt\n", ssid.c_str());
    delay(800);
    ESP.restart();
  });
  ...
  diagnosticSerial.printf("OPPSETT | Koble telefonen til %s | åpne http://192.168.4.1\n", provisioningApName.c_str());
}
...
void handleWifiResetButton() {
  ...
  diagnosticSerial.println("OPPSETT | Wi-Fi er nullstilt med knappen. Starter oppsettsmodus.");
  Serial.flush();
  delay(500);
  ESP.restart();
}
...
void startWifiAttempt(bool resetRadio) {
  if (resetRadio) {
    diagnosticSerial.println("Nullstiller Wi-Fi-radio før nytt forsøk.");
...
void ensureWifi() {
  if (provisioningActive) {
    ...
    if (millis() - provisioningStartedAtMs >= WIFI_PROVISIONING_TIMEOUT_MS) {
      diagnosticSerial.println("OPPSETT | Tidsgrense nådd. Starter enheten på nytt.");
      ESP.restart();
    }
    return;
  }
  if (WiFi.status() == WL_CONNECTED) {
    if (wifiConnectedAtMs == 0) {
      ...
      diagnosticSerial.printf("Wi-Fi OK. IP: %s\n", WiFi.localIP().toString().c_str());
  ...
  if (!wifiOutageActive) {
    ...
    diagnosticSerial.println("Wi-Fi frakoblet. Starter kontrollert gjenoppretting.");
  }

  if (now - wifiDisconnectedAtMs >= WIFI_DEVICE_RESTART_MS) {
    diagnosticSerial.println("Wi-Fi har vÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦rt borte i 10 minutter. Starter ESP pÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¥ nytt.");
    Serial.flush();
    delay(100);
    ESP.restart();
  }
...
void printNetworkFailureDetails(
  ...
  if (dnsResult == 1) {
    diagnosticSerial.printf(" (%s)", resolved.toString().c_str());
  }
...
void appendDirectionLabel(String& labels, const String& destination) {
  const String clean = destination.substring(0, 60);
  if (clean.length() == 0) return;

  int start = 0;
  while (start < labels.length()) {
    int end = labels.indexOf(" / ", start);
    if (end < 0) end = labels.length();
    if (labels.substring(start, end) == clean) return;
    start = end + 3;
  }
...
void printFrameSimulation(uint32_t sequence, uint16_t activeCount) {
  ...
  if (activeCount == 0) {
    diagnosticSerial.println("  Ingen aktive LED-er.");
    return;
  }
  ...
  for (uint16_t id = 0; id < LED_COUNT; id++) {
    ...
    const String direction = candidateDirectionLabels[id].length() > 0
      ? "mot " + candidateDirectionLabels[id]
      : "retning ukjent";
    diagnosticSerial.printf(
      "  LED %03u | %-11s | %-28s | RGB %u,%u,%u | brightness %u | vogn %s\n",
      id,
      ...
      pixel.blue,
      pixel.brightness,
      candidateVehicleIds[id].length() ? candidateVehicleIds[id].c_str() : "ukjent"
    );
  }
}
...
bool parseAndValidateFrame(
  ...
  for (uint16_t id = 0; id < LED_COUNT; id++) {
    candidateDirectionLabels[id] = "";
    candidateVehicleIds[id] = "";
  }
  ...
  for (JsonObject item : leds) {
    ...
    appendDirectionLabel(candidateVehicleIds[id], String((const char*)(item["vehicle"]["id"] | "")));
    ...
    if (!occupants.isNull()) {
      for (JsonObject occupant : occupants) {
        ...
        appendDirectionLabel(candidateVehicleIds[id], String((const char*)(occupant["id"] | "")));
        appendDirectionLabel(candidateDirectionLabels[id], String((const char*)(occupant["destination"] | "")));
    ...
    if (candidateDirectionLabels[id].length() == 0) {
      appendDirectionLabel(candidateDirectionLabels[id], String((const char*)(item["vehicle"]["destination"] | "")));
    }
    activeCount++;
  }
...
bool fetchFrameAttempt(int retryIndex) {
  ...
  if (!receiveFeedBody(body, httpStatus, error)) {
    diagnosticSerial.printf("Feed-mottaksfeil: %s\n", error.c_str());
  ...
  if (!parseAndValidateFrame(
    ...
    diagnosticSerial.printf("Feed-valideringsfeil: %s\n", error.c_str());
...
void fetchFrame() {
  ...
  if (consecutiveFeedFailures >= FEED_FAILURES_BEFORE_WIFI_RESET) {
    diagnosticSerial.println("Gjentar TLS-feil. Kobler Wi-Fi kontrollert til på nytt.");
    WiFi.disconnect(false, false);
    wifiConnectedAtMs = 0;
  }
}
...
bool sendHealthStatus(unsigned long now, uint32_t freeHeap) {
  ...
  if (strlen(TRANSITCORE_DEVICE_TOKEN) == 0) {
    diagnosticSerial.println("STATUS | token IKKE konfigurert");
    return true;
  }
  ...
  JsonArray queuedErrors = document.createNestedArray("errorQueue");
  for (uint8_t index = 0; index < deviceErrorQueueCount; index++) {
    JsonObject event = queuedErrors.createNestedObject();
    event["id"] = deviceErrorQueue[index].id;
    event["code"] = deviceErrorQueue[index].code;
    event["detail"] = deviceErrorQueue[index].detail;
    event["occurredAtUptimeSeconds"] = deviceErrorQueue[index].occurredAtUptimeSeconds;
  ...
  if (!http.begin(client, endpoint)) {
    diagnosticSerial.println("STATUS | HTTP begin feilet");
    return true;
  }
  http.addHeader("Content-Type", "application/json");
  http.addHeader("Authorization", "Bearer " + String(TRANSITCORE_DEVICE_TOKEN));
  const int status = http.POST(payload);
  if (status == 200 && http.getSize() <= 512) acceptLogLease(http.getString());
  http.end();
  diagnosticSerial.printf("STATUS | HTTP %d\n", status);
  const bool delivered = status >= 200 && status < 300;
  if (delivered) {
    deviceErrorReportPending = false;
    if (deviceErrorQueueCount > 0) clearDeliveredDeviceErrors();
  }
  return delivered;
}


void sendRemoteLog(unsigned long now) {
  if (!remoteLogUntilMs || int32_t(remoteLogUntilMs-now) <= 0 ||
      WiFi.status()!=WL_CONNECTED || now-remoteLogLastAttemptMs<10000UL) return;
  remoteLogLastAttemptMs=now;
  DynamicJsonDocument doc(8192);
  doc["bootId"]=diagnosticBootId;
  doc["uptimeSeconds"]=now/1000UL;
  const uint32_t sent=diagnosticSerial.snapshot(doc.createNestedArray("lines"),remoteLogSent);
  if (doc.overflowed()) return;
  String payload; serializeJson(doc,payload);
  String endpoint=statusEndpoint(); endpoint.remove(endpoint.length()-6); endpoint += "logs";
  ...
  if (code==200 && http.getSize()<=512) { remoteLogSent=sent; acceptLogLease(http.getString()); }
  else if (code==403 || code==401 || code==404) remoteLogUntilMs=0;
  http.end();
}
...
void enforceTtl() {
  if (ttlExpiryReportPending) {
    ttlExpiryReportPending = false;
    recordDeviceError("FRAME_EXPIRED", "Siste gyldige LED-frame overskred TTL i LED-vakthunden");
  }
  ...
  portEXIT_CRITICAL(&frameMutex);
  clearHardware();
  diagnosticSerial.println("LED-frame utlÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¸pt. Tavlen er slukket.");
}
...
void checkForOtaUpdate(unsigned long now) {
  ...
  manifestHttp.addHeader("X-TransitCore-Chip", OTA_CHIP);
  manifestHttp.addHeader("X-TransitCore-Gpio", String(LED_DATA_PIN));
  manifestHttp.addHeader("X-TransitCore-Leds", String(LED_COUNT));
  manifestHttp.addHeader("X-TransitCore-Physical-Leds", String(TRANSITCORE_PHYSICAL_LED_COUNT));
  const int status = manifestHttp.GET();
  if (status == HTTP_CODE_NO_CONTENT || status == HTTP_CODE_NOT_MODIFIED) {
    manifestHttp.end();
    diagnosticSerial.println("OTA | ingen nyere firmware.");
    return;
  }
  ...
  if (!newerFirmwareVersion(version)) {
    diagnosticSerial.printf("OTA | versjon %s gir ingen oppdatering fra %s.\n", version.c_str(), TRANSITCORE_FIRMWARE_VERSION);
    return;
  }
  ...
  const String deviceId = manifest["deviceId"] | "";
  const String chip = manifest["chip"] | "";
  const String md5 = manifest["md5"] | "";
  const uint32_t size = manifest["size"] | 0U;
  const esp_partition_t* partition = esp_ota_get_next_update_partition(nullptr);
  bool validDigest = md5.length() == 32;
  for (size_t i=0; i<md5.length(); ++i) if (!isxdigit(md5[i])) validDigest=false;
  if (deviceId != TRANSITCORE_DEVICE_ID || chip != OTA_CHIP ||
      boardProfile != EXPECTED_BOARD_PROFILE ||
      (manifest["gpio"] | -1) != LED_DATA_PIN ||
      (manifest["ledCount"] | 0) != LED_COUNT ||
      (manifest["physicalLedCount"] | 0) != TRANSITCORE_PHYSICAL_LED_COUNT ||
      !validDigest || !partition || size == 0 || size > partition->size) {
    recordDeviceError("OTA_TARGET", "Oppdateringen passer ikke enhet, GPIO eller OTA-partisjon");
    return;
  }
  httpUpdate.setMD5sum(md5.c_str());
  diagnosticSerial.printf("OTA | installerer %s over verifisert TLS.\n", version.c_str());
  ...
  if (result == HTTP_UPDATE_FAILED) {
    ...
    diagnosticSerial.println("OTA | binærendepunktet meldte ingen oppdatering.");
  }
}
...
void setup() {
  Serial.begin(115200);
  diagnosticSerial.begin();
  diagnosticBootId = String(esp_random(), HEX) + String(esp_random(), HEX);
  delay(250);
  esp_ota_img_states_t otaState;
  otaTrialBoot = esp_ota_get_state_partition(esp_ota_get_running_partition(), &otaState) == ESP_OK &&
    otaState == ESP_OTA_IMG_PENDING_VERIFY;
  if (otaTrialBoot) diagnosticSerial.println("OTA | prøveoppstart: venter på gyldig feed og LED-oppgave.");

  loadDeviceErrorQueue();
  ...
  diagnosticSerial.printf("TransitCore Universal Board Client v1.2.13 starter | kompatibel stabil ESP32-S3 LED-driver | hardwareprofil låser LED-datapin | vedvarende feilkø %u/10 | Wi-Fi-reset: hold BOOT i 5 sekunder.\n", deviceErrorQueueCount);
  diagnosticSerial.printf(
    "Board %s | %u tavle-LED-er | %u fysiske stripe-LED-er | datapin GPIO %u | hardware %s\n",
  ...
  diagnosticSerial.printf("LED-frame isolasjonstest: %s\n",
    TRANSITCORE_LED_FRAME_ISOLATION_TEST ? "JA" : "NEI");
  diagnosticSerial.printf("OTA: %s | firmware %s\n",
    TRANSITCORE_OTA_ENABLED ? "AKTIV" : "KLAR, MEN AVSLÅTT",
    TRANSITCORE_FIRMWARE_VERSION);
  diagnosticSerial.println("Skriv HELP i Serial Monitor for visningskommandoer.");
  printVisualMode();
}
...
void loop() {
  ...
  const unsigned long now = millis();
  if (otaTrialBoot) {
    const bool rendererAlive = !LED_HARDWARE_ENABLED || (lastLedTaskAliveMs != 0 && now-lastLedTaskAliveMs<2000);
    if (now>60000 && successfulFeedPolls>=2 && hasValidFrame && !ttlExpired && rendererAlive) {
      if (esp_ota_mark_app_valid_cancel_rollback()==ESP_OK) {
        otaTrialBoot=false;
        diagnosticSerial.println("OTA | ny firmware bekreftet etter prøveoppstart.");
      }
    } else if (now>5UL*60UL*1000UL) {
      diagnosticSerial.println("OTA | prøveoppstart feilet, gjenoppretter forrige firmware.");
  ...
  enforceTtl();
  reportHealth();
  sendRemoteLog(millis());
  checkForOtaUpdate(millis());
  delay(20);
}

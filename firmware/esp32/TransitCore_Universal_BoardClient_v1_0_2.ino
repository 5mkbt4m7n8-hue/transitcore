...
// TransitCore Universal Board Client v1.0.2
// One stable ESP32 engine; board_config.h selects the physical board.
// v1.0.2 adds controlled Wi-Fi recovery; frame and LED behavior are unchanged.
...
void startWifiAttempt(bool resetRadio) {
  if (resetRadio) {
    Serial.println("Nullstiller Wi-Fi-radio fÃƒÆ’Ã‚Â¸r nytt forsÃƒÆ’Ã‚Â¸k.");
  ...
  Serial.printf(
    "Wi-Fi-forsÃƒÆ’Ã‚Â¸k %u startet | neste tidligst om %lu s\n",
    wifiAttemptCount,
...
void ensureWifi() {
  ...
  if (now - wifiDisconnectedAtMs >= WIFI_DEVICE_RESTART_MS) {
    Serial.println("Wi-Fi har vÃƒÆ’Ã‚Â¦rt borte i 10 minutter. Starter ESP pÃƒÆ’Ã‚Â¥ nytt.");
    Serial.flush();
    delay(100);
    ESP.restart();
  }
...
bool parseAndValidateFrame(
  ...
  if (ageSeconds > (int64_t)ttlSeconds) {
    error = "frame er utlÃƒÆ’Ã‚Â¸pt ved mottak: " + String((long)ageSeconds) + " s";
    return false;
  }
  ...
  for (JsonObject item : leds) {
    ...
    if (
      ...
      error = "ugyldig LED-oppfÃƒÆ’Ã‚Â¸ring";
      return false;
    }
...
void enforceTtl() {
  ...
  clearHardware();
  Serial.println("LED-frame utlÃƒÆ’Ã‚Â¸pt. Tavlen er slukket.");
}
...
void setup() {
  ...
  Serial.println("TransitCore Universal Board Client v1.0.2 starter.");
  Serial.printf(
    "Board %s | %u LED-er | hardware %s\n",
    EXPECTED_BOARD_PROFILE,
    LED_COUNT,
    LED_HARDWARE_ENABLED ? "PÃƒÆ’Ã¢â‚¬Â¦" : "AV"
  );
}

...
const recedingApproachFrame={...tramFrame,leds:[{...tramFrame.leds[0],vehicle:{...tramFrame.leds[0].vehicle,distanceMeters:30}}]};
let recedingApproach=applyMotionLifecycle({...recedingApproachFrame,leds:[{...recedingApproachFrame.leds[0],vehicle:{...recedingApproachFrame.leds[0].vehicle,distanceMeters:5}}]}, {}, now, 10000);
recedingApproach=applyMotionLifecycle(recedingApproachFrame,recedingApproach.state,now+10000,10000);
assert.equal(recedingApproach.frame.leds[0].state,"APPROACHING","en vogn som bare har nærmet seg må ikke bli PASSED når GPS-avstanden øker");
assert.equal(recedingApproach.frame.leds[0].lifecycle,undefined);

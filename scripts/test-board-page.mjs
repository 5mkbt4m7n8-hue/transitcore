...
assert.match(inline,/\["oslo-metro-wizard-separate","trondheim-bus-board","grakallbanen-board"\]\.includes\(board\.id\)/);
assert.match(inline,/"trondheim-bus-board":"\.\.\/\.\.\/config\/hardware\/trondheim-bus-board-hardware\.json"/);
assert.match(inline,/entry\.state\|\|item\.lifecycle\|\|item\.state/);
assert.match(inline,/String\(value\.line\.color\|\|color\(value\)\)\.toLowerCase\(\)/);

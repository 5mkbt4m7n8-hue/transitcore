...
const firmwareSource=fs.readFileSync(new URL("../firmware/esp32/TransitCore_Universal_BoardClient_v1_2_10.ino",import.meta.url),"utf8");
...
assert.equal(api.FIRMWARE_VERSION,"1.2.10");
assert.equal(api.FIRMWARE_FILE,"TransitCore_Universal_BoardClient_v1_2_10.ino");
...
assert.match(firmwareSource,/document\["firmware"\] = "1\.2\.10"/);
...
assert.match(firmwareSource,/strip\.end\(\);[\s\S]*strip\.begin\(\);/,"manuell bussgjenoppretting skal frigjøre RMT før ny initialisering");
...
assert.equal(result.filename,"test-board-esp32-v1.2.10.zip");
...
console.log("ESP package builder OK: 4 safe files for Board Client v1.2.10");

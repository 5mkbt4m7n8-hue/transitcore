...
const firmwareSource=fs.readFileSync(new URL("../firmware/esp32/TransitCore_Universal_BoardClient_v1_1_4.ino",import.meta.url),"utf8");
...
assert.equal(api.FIRMWARE_VERSION,"1.1.4");
assert.equal(api.FIRMWARE_FILE,"TransitCore_Universal_BoardClient_v1_1_4.ino");
...
assert.match(firmwareSource,/document\["firmware"\] = "1\.1\.4"/);
assert.match(firmwareSource,/setHandshakeTimeout/);
assert.match(firmwareSource,/FEED_FAILURES_BEFORE_WIFI_RESET/);
assert.match(firmwareSource,/millis\(\) - lastValidFrameAtMs > frameTtlMs/);
...
assert.equal(result.filename,"test-board-esp32-v1.1.4.zip");
...
console.log("ESP package builder OK: 4 safe files for Board Client v1.1.4");

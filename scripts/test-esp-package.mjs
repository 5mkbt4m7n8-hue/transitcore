...
const firmwareSource=fs.readFileSync(new URL("../firmware/esp32/TransitCore_Universal_BoardClient_v1_2_13.ino",import.meta.url),"utf8");
...
const grakallPrototypeHardware=JSON.parse(fs.readFileSync(new URL("../config/hardware/grakallbanen-prototype-board-hardware.json",import.meta.url),"utf8"));
const context={};context.globalThis=context;vm.runInNewContext(source,context);
const api=context.TransitCoreEspPackage;

assert.equal(api.FIRMWARE_VERSION,"1.2.13");
assert.equal(api.FIRMWARE_FILE,"TransitCore_Universal_BoardClient_v1_2_13.ino");
...
assert.match(firmwareSource,/document\["firmware"\] = "1\.2\.13"/);
...
assert.match(firmwareSource,/DEVICE_ERROR_QUEUE_CAPACITY = 10/);
assert.match(firmwareSource,/loadDeviceErrorQueue\(\)/);
...
assert.match(firmwareSource,/recordDeviceError\("WIFI_DISCONNECTED"/);
assert.match(firmwareSource,/createNestedObject\("lastError"\)/);
assert.match(firmwareSource,/AMBIENT_BRIGHTNESS = 9/);
assert.match(firmwareSource,/command == "AMBIENT ON"/);
assert.match(firmwareSource,/command == "TRAINS WARM"/);
assert.match(firmwareSource,/void cycleVisualMode\(\)/);
assert.match(firmwareSource,/void cycleMasterBrightness\(\)/);
assert.match(firmwareSource,/masterBrightnessPercent == 100\) masterBrightnessPercent = 50/);
assert.match(firmwareSource,/masterBrightnessPercent == 50\) masterBrightnessPercent = 10/);
assert.match(firmwareSource,/command == "BRIGHTNESS NEXT"/);
assert.match(firmwareSource,/255UL \* 255UL \* 100UL/);
assert.match(firmwareSource,/warmWhiteVehiclesEnabled && pixel\.state != LED_PARKED/,"PARKED skal fortsatt være rød i varmhvit togmodus");
assert.match(firmwareSource,/if \(strcmp\(value, "PARKED"\) == 0\) return LED_PARKED;/,"PARKED skal kunne mottas fra Worker");
assert.match(firmwareSource,/if \(state == LED_PARKED\) return 4;/,"PARKED skal ha høyeste prioritet");
...
assert.match(firmwareSource,/wifiPreferences\.remove\("forceSetup"\)/);
assert.match(firmwareSource,/handleWifiResetButton\(\);/);
assert.match(firmwareSource,/TRANSITCORE_PHYSICAL_LED_COUNT/);
assert.match(firmwareSource,/TRANSITCORE_LED_FRAME_ISOLATION_TEST/);
assert.match(firmwareSource,/#include <HTTPUpdate\.h>/);
assert.match(firmwareSource,/TRANSITCORE_OTA_ENABLED/);
assert.match(firmwareSource,/void checkForOtaUpdate\(unsigned long now\)/);
assert.match(firmwareSource,/binaryUrl\.startsWith\("https:\/\/"\)/);
assert.match(firmwareSource,/httpUpdate\.update\(binaryClient/);
assert.match(firmwareSource,/setHandshakeTimeout/);
assert.match(firmwareSource,/FEED_FAILURES_BEFORE_WIFI_RESET/);
assert.match(firmwareSource,/if \(millis\(\) - lastValidFrameAtMs <= frameTtlMs\) return;/);
assert.match(firmwareSource,/xTaskCreatePinnedToCore\([\s\S]*ledRenderTask/);
assert.match(firmwareSource,/LED-frame utløpt i LED-oppgaven/);
const recoverySource=firmwareSource.match(/void recoverLedBus\(const char\* reason\) \{[\s\S]*?\n\}/)?.[0] || "";
assert.doesNotMatch(recoverySource,/strip\.(?:end|begin)\(/,"bussgjenoppretting skal være kompatibel og ikke initialisere NeoPixel-driveren på nytt");
assert.match(recoverySource,/strip\.clear\(\);[\s\S]*strip\.show\(\);/,"bussgjenoppretting skal sende en kontrollert mørk frame");
assert.doesNotMatch(firmwareSource,/recoverLedBus\("PERIODISK VAKTHUND"\)/,"frisk NeoPixel-driver skal ikke restartes periodisk");
assert.match(firmwareSource,/command == "LED RECOVER"/);
assert.match(firmwareSource,/HTTP_RESPONSE_TIMEOUT_MS = 7000/);
...
const result=api.createFiles({
  ...
  hardware:{leds:{count:2,dataPin:14}},
  boardConfig:"#pragma once\nconst uint8_t LED_DATA_PIN = 2;\n",
  firmware:"void setup(){}\nvoid loop(){}\n"
});
assert.equal(result.files.length,4);
assert.equal(result.filename,"test-board-esp32-v1.2.13.zip");
assert.deepEqual(Array.from(result.files,file=>file.name),[
  ...
  "TransitCore_test_board_ESP32/secrets.h",
...
assert.match(result.files[1].content,/TRANSITCORE_PHYSICAL_LED_COUNT 2/);
assert.match(result.files[1].content,/const uint8_t LED_DATA_PIN = 14;/,"maskinvareprofilen skal overstyre gammel GPIO i prosjektpakken");
assert.doesNotMatch(result.files[1].content,/const uint8_t LED_DATA_PIN = 2;/,"gammel GPIO må ikke følge med i ny ESP-pakke");
assert.match(result.files[3].content,/Datapin: GPIO 14/);
const longStrip=api.createFiles({board:{id:"test-board",leds:{count:2}},hardware:{leds:{count:2}},boardConfig:"#pragma once\n",firmware:"void setup(){}",physicalLedCount:300});
assert.match(longStrip.files[1].content,/TRANSITCORE_PHYSICAL_LED_COUNT 300/);
assert.match(longStrip.files[1].content,/LED_DATA_PIN = 14;/);
assert.match(firmwareSource,/vogn %s/);
assert.match(firmwareSource,/candidateVehicleIds\[LED_COUNT\]/);
assert.match(longStrip.files[1].content,/TRANSITCORE_LED_FRAME_ISOLATION_TEST 0/);
assert.match(longStrip.files[1].content,/TRANSITCORE_OTA_ENABLED 1/);
assert.match(longStrip.files[1].content,/\/v1\/firmware\/manifest/);
assert.match(longStrip.files[3].content,/OTA-beredskap/);
const isolated=api.createFiles({board:{id:"test-board",leds:{count:2}},hardware:{leds:{count:2}},boardConfig:"#pragma once\n",firmware:"void setup(){}",physicalLedCount:150,isolationTest:true});
assert.match(isolated.files[1].content,/TRANSITCORE_LED_FRAME_ISOLATION_TEST 1/);
...
console.log("ESP package builder OK: 4 safe files for Board Client v1.2.13");
